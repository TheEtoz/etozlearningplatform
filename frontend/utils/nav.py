...
def render_student_sidebar() -> None:
    ...
    with st.sidebar:
        ...
        st.page_link("pages/CodingPath.py", label="Coding Path", icon="💻")
        st.page_link("pages/Profile.py", label="Profile", icon="👤")
        st.divider()
        if st.button("Log out", use_container_width=True, key="student_logout"):
            clear_auth_session()
...
def render_teacher_sidebar(active: str | None = None) -> None:
    ...
    with st.sidebar:
        ...
        st.caption("Content")
        st.page_link("pages/_QuestionBank.py", label="Question Bank", icon="📚")
        st.page_link("pages/_QuizManager.py", label="Quizzes", icon="📝")
        st.page_link("pages/_PathManager.py", label="Coding Path", icon="🗺️")
        st.caption("Account")
        st.page_link("pages/Profile.py", label="Profile", icon="👤")
        st.divider()
        if st.button("Log out", use_container_width=True, key="teacher_logout"):
            clear_auth_session()
            st.switch_page("app.py")
